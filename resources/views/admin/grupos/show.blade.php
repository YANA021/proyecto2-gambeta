...
<head>
    ...
    <title>Grupo #{{ $grupo->id }} - Gambeta</title>
    ...
    <div class="mx-auto max-w-4xl px-6 py-10">
        <header class="mb-6 flex items-center justify-between gap-3">
            <div>
                ...
                <h1 class="text-2xl font-bold">Grupo #{{ $grupo->id }}</h1>
            </div>
            <a href="{{ route('grupos.index') }}" class="text-sm font-semibold text-slate-600 hover:text-slate-800">Volver</a>
        </header>

        <div class="rounded-xl border border-slate-100 bg-white p-6 shadow-sm space-y-4">
            <div>
                <p class="text-sm text-slate-500">Nombre</p>
                <p class="text-lg font-semibold text-slate-900">{{ $grupo->nombre }}</p>
            </div>
