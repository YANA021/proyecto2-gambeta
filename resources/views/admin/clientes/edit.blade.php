...
<head>
    ...
    <title>Editar Cliente - Gambeta</title>
    ...
    <div class="mx-auto max-w-3xl px-6 py-10">
        ...
        <div class="rounded-xl border border-slate-100 bg-white p-6 shadow-sm">
            <form action="{{ route('clientes.update', $cliente) }}" method="POST" class="space-y-4">
                ...
                <div>
                    <label class="text-sm font-semibold text-slate-700">Grupo (opcional)</label>
                    <select name="grupo_id"
                            class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-slate-400 focus:outline-none">
                        <option value="">Sin grupo</option>
                        @foreach($grupos as $id => $nombre)
                            <option value="{{ $id }}" @selected(old('grupo_id', $cliente->grupo_id) == $id)>{{ $nombre }}</option>
                        @endforeach
                    </select>
