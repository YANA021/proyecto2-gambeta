<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Cancha #{{ $cancha->id }} - Gambeta</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>
<body class="bg-slate-50 text-slate-900">
    <div class="mx-auto max-w-4xl px-6 py-10">
        <header class="mb-6 flex items-center justify-between gap-3">
            <div>
                <p class="text-xs uppercase tracking-wide text-slate-500">Detalle</p>
                <h1 class="text-2xl font-bold">Cancha #{{ $cancha->id }}</h1>
            </div>
            <a href="{{ route('canchas.index') }}" class="text-sm font-semibold text-slate-600 hover:text-slate-800">Volver</a>
        </header>

        <div class="space-y-6">
            <div class="rounded-xl border border-slate-100 bg-white p-6 shadow-sm">
                <div class="grid gap-6 md:grid-cols-[2fr,1fr]">
                    <div class="space-y-3">
                        <p class="text-sm text-slate-500">Nombre</p>
                        <p class="text-lg font-semibold text-slate-900">{{ $cancha->nombre }}</p>

                        <p class="text-sm text-slate-500">Tipo</p>
                        <p class="font-semibold">{{ $cancha->tipoCancha->nombre ?? '—' }}</p>

                        <p class="text-sm text-slate-500">Precio por hora</p>
                        <p class="font-semibold">${{ number_format($cancha->precio_hora, 2) }}</p>

                        <p class="text-sm text-slate-500">Estado</p>
                        @if($cancha->disponible)
                            <span class="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">Disponible</span>
                        @else
                            <span class="rounded-full bg-rose-50 px-3 py-1 text-xs font-semibold text-rose-700">No disponible</span>
                        @endif
                    </div>

                    <div>
                        <div class="overflow-hidden rounded-xl border border-slate-100 bg-slate-100">
                            @if($cancha->foto)
                                <img src="{{ Storage::url($cancha->foto) }}" alt="Foto cancha"
                                     class="h-56 w-full object-cover">
                            @else
                                <div class="flex h-56 items-center justify-center text-sm text-slate-500">
                                    Sin foto
                                </div>
                            @endif
                        </div>
                    </div>
                </div>

                <div class="mt-6 flex flex-wrap gap-3 text-sm font-semibold">
                    <a href="{{ route('canchas.edit', $cancha) }}"
                       class="rounded-lg bg-slate-900 px-4 py-2 text-white hover:bg-slate-800">Editar</a>
                    <form action="{{ route('canchas.destroy', $cancha) }}" method="POST"
                          onsubmit="return confirm('¿Eliminar esta cancha?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="rounded-lg bg-rose-500 px-4 py-2 text-white hover:bg-rose-600">
                            Eliminar
                        </button>
                    </form>
                </div>
            </div>

            <div class="rounded-xl border border-slate-100 bg-white p-6 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs uppercase tracking-wide text-slate-500">Disponibilidad</p>
                        <h2 class="text-lg font-semibold text-slate-900">Calendario</h2>
                        <p class="text-sm text-slate-500">Aquí podrás visualizar los bloques reservados para esta cancha.</p>
                    </div>
                    <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">Próximamente</span>
                </div>
                <div class="mt-4 rounded-lg border border-dashed border-slate-200 bg-slate-50 px-4 py-10 text-center text-sm text-slate-500">
                    Integra tu calendario de reservas aquí. Por ahora, no hay datos de disponibilidad cargados.
                </div>
            </div>
        </div>
    </div>
</body>
</html>
