...
<head>
    ...
    <title>Nueva Cancha - Gambeta</title>
    ...
    <div class="mx-auto max-w-3xl px-6 py-10">
        <header class="mb-6 flex items-center justify-between">
            <div>
                ...
                <h1 class="text-2xl font-bold">Nueva cancha</h1>
            </div>
        ...
        <div class="rounded-xl border border-slate-100 bg-white p-6 shadow-sm">
            <form action="{{ route('canchas.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <select name="tipo_id" required
                        ...
                        @foreach($tipos as $id => $nombre)
                            <option value="{{ $id }}" @selected(old('tipo_id') == $id)>{{ $nombre }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="precio_hora" value="{{ old('precio_hora') }}" required
                           class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-slate-400 focus:outline-none">
