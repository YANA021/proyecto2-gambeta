...
<head>
    ...
    <title>Editar Reserva - Gambeta</title>
            ...
            <form action="{{ route('reservas.update', $reserva) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="cancha_id" required
                        ...
                        @foreach($canchas as $id => $nombre)
                            <option value="{{ $id }}" @selected(old('cancha_id', $reserva->cancha_id) == $id)>{{ $nombre }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="cliente_id" required
                        ...
                        @foreach($clientes as $id => $nombre)
                            <option value="{{ $id }}" @selected(old('cliente_id', $reserva->cliente_id) == $id)>{{ $nombre }}</option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="date" name="fecha" value="{{ old('fecha', optional($reserva->fecha)->format('Y-m-d')) }}" required
                               class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-slate-400 focus:outline-none">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="duracion_horas" min="1" value="{{ old('duracion_horas', $reserva->duracion_horas) }}" required
                               class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-slate-400 focus:outline-none">
                    ...
                    <div>
                        ...
                        <select name="estado_id" required
                            ...
                            @foreach($estados as $id => $nombre)
                                <option value="{{ $id }}" @selected(old('estado_id', $reserva->estado_id) == $id)>{{ $nombre }}</option>
                            @endforeach
                        </select>
                ...
                <p class="text-xs text-slate-500">El precio se recalculará automáticamente al guardar (precio de la cancha x horas).</p>
