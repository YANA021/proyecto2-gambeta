...
<head>
    ...
    <title>Estados de Reserva - Gambeta</title>
    ...
    <div class="mx-auto max-w-6xl px-6 py-10">
        <header class="mb-6 flex items-center justify-between gap-4">
            <div>
                ...
                <h1 class="text-2xl font-bold">Estados de reserva</h1>
            </div>
            <a href="{{ route('estados_reserva.create') }}"
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($estados as $estado)
                        <tr class="hover:bg-slate-50">
                            <td class="px-4 py-3 font-semibold text-slate-900">#{{ $estado->id }}</td>
                            <td class="px-4 py-3">{{ $estado->nombre }}</td>
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap items-center gap-2 text-xs font-semibold">
                                    <a href="{{ route('estados_reserva.show', $estado) }}"
                                    ...
                                    <form action="{{ route('estados_reserva.destroy', $estado) }}" method="POST"
                                          onsubmit="return confirm('¿Eliminar este estado?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-4 py-6 text-center text-slate-500">No hay estados registrados.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
